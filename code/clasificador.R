...
for (i in 1:length(conjuntos)) {
  ...
  colnames(test)[4] <- "residual.sugar"
  colnames(test)[5] <- "chlorides"
  colnames(test)[6] <- "free.sulfur.dioxide"
  colnames(test)[7] <- "bound.sulfur.dioxide"
  colnames(test)[8] <- "pH"
...
data_0 <- read.csv("datos-no-discretizados.csv", header=TRUE)
...
sink("reglas-sin-discretizar.txt")
print(summary(model))
sink()
